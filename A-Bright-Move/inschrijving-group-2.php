...
<div class="container-fluid">
	...
	<?php for($x = 1; $x <= $_GET['aantal']; $x++) {?>
		<h3>Aanmelding <?php echo "$x" ?></h3>
		<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>?aantal=<?php echo "$aantal" ?>" name="aanmelding<?php echo "$x" ?>">
			<div class="row">
				<div class="col-sm-2 form-group">
					<p>voornaam: </p>
				</div>
				<div class="col-sm-4 form-group">
					<input class="form-control <?php if(!empty($voornaamErr)){echo "error";} ?>" type="text" name="voornaam<?php echo "$x" ?>" value="<?php echo isset($_POST['voornaam' . $x]) ? $_POST['voornaam' . $x] : '' ?>">
			...
			<div class="row">
				...
				<div class="col-sm-4 form-group">
					<input class="form-control <?php if(!empty($achternaamErr)){echo "error";} ?>" type="text" name="achternaam<?php echo "$x" ?>" value="<?php echo isset($_POST['achternaam' . $x]) ? $_POST['achternaam' . $x] : '' ?>">
			...
			<div class="row">
				...
				<div class="col-sm-4 form-group" style="display: flex; justify-content: space-between; max-width: 330px;">
					<select class="form-control date <?php if(!empty($geboorteErr)){echo "error";} ?>" name="dag<?php echo "$x" ?>">
						<option value="<?php echo isset($_POST['dag'.$x]) ? $_POST['dag'.$x] : '' ?>"><?php echo isset($_POST['dag'.$x]) ? $_POST['dag'.$x] : 'Dag' ?></option>
							<?php for ($day = 1; $day <= 31; $day++) { ?>
								<option value="<?php echo strlen($day)==1 ? '0'.$day : $day; ?>"><?php echo strlen($day)==1 ? '0'.$day : $day; ?></option>
							<?php } ?>
					</select>
					<select class="form-control date <?php if(!empty($geboorteErr)){echo "error";} ?>" name="maand<?php echo "$x" ?>">
						<option value="<?php echo isset($_POST['maand' . $x]) ? $_POST['maand' . $x] : '' ?>"><?php echo isset($_POST['maand' . $x]) ? $_POST['maand' . $x] : 'Maand' ?></option>
							<?php for ($month = 1; $month <= 12; $month++) { ?>
								<option value="<?php echo strlen($month)==1 ? '0'.$month : $month; ?>"><?php echo strlen($month)==1 ? '0'.$month : $month; ?></option>
							<?php } ?>
					</select>
					<select class="form-control date <?php if(!empty($geboorteErr)){echo "error";} ?>" name="jaar<?php echo "$x" ?>">
						<option value="<?php echo isset($_POST['jaar' . $x]) ? $_POST['jaar' . $x] : '' ?>"><?php echo isset($_POST['jaar' . $x]) ? $_POST['jaar' . $x] : 'Jaar' ?></option>
			...
			<?php if (time() < strtotime('+18 years', strtotime($geboortedatum))) { $minor = "ouder" ?>
				...
				<div class="row">
					...
					<div class="col-sm-4 form-group">
						<input class="form-control <?php if(!empty($voornaamOuderErr)){echo "error";} ?>" type="text" name="voornaam-ouder<?php echo "$x" ?>" value="<?php echo isset($_POST['voornaam-ouder' . $x]) ? $_POST['voornaam-ouder' . $x] : '' ?>">
				...
				<div class="row">
					...
					<div class="col-sm-4 form-group">
						<input class="form-control <?php if(!empty($achternaamOuderErr)){echo "error";} ?>" type="text" name="achternaam-ouder<?php echo "$x" ?>" value="<?php echo isset($_POST['achternaam-ouder' . $x]) ? $_POST['achternaam-ouder' . $x] : '' ?>">
			...
			<div class="row">
				...
				<div class="col-sm-4 form-group">
					<input class="form-control <?php if(!empty($emailErr)){echo "error";} ?>" type="text" name="email<?php echo "$x" ?>" value="<?php echo isset($_POST['email' . $x]) ? $_POST['email' . $x] : '' ?>">
			...
			<div class="row">
				...
				<div class="col-sm-4 form-group">
					<input class="form-control <?php if(!empty($telefoonErr)){echo "error";} ?>" type="text" name="telefoonnummer<?php echo "$x" ?>" value="<?php echo isset($_POST['telefoonnummer' . $x]) ? $_POST['telefoonnummer' . $x] : '' ?>">
			...
			<div class="row">
				<div class="col-sm-6 form-group">
					...
					<label for="voorwaarden">Ik ga akkoord met de <a href="doc/Algemene-voorwaarden.pdf" target="_blanc">algemene voorwaarden</a>.</label><br>
					<span class="error"><?php echo $termsErr;?></span>
				</div>
			</div>
		</form>
	<?php }?> 
	<input type="button" value="Verstuur" onclick="submitForms()" />
</div>

<?php
for($x = 1; $x <= $_GET['aantal']; $x++) {
	if (empty($_POST["voornaam".$x])) {
		$voornaamErr = "vul een geldige voornaam in";
	} else {
		$voornaam = test_input($_POST["voornaam".$x]);
	}
	  
	if (empty($_POST["achternaam".$x])) {
		$achternaamErr = "vul een geldige achternaam in";
	} else {
		$achternaam = test_input($_POST["achternaam".$x]);
	}
	  
	if (empty($_POST["dag".$x]) || empty($_POST["maand".$x]) || empty($_POST["jaar".$x])) {
		$geboorteErr = "vul een geldige geboortedatum in";
	} else {
		$geboortedatum = test_input($_POST["dag".$x]) . "-" . test_input($_POST["maand".$x]) . "-" . test_input($_POST["jaar".$x]);
	}
	  
	if (empty($_POST["voornaam-ouder".$x])) {
		$voornaamOuderErr = "vul een geldige voornaam in";
	} else {
		$voornaamOuder = test_input($_POST["voornaam-ouder".$x]);
	}
	  
	if (empty($_POST["achternaam-ouder".$x])) {
		$achternaamOuderErr = "vul een geldige achternaam in";
	} else {
		$achternaamOuder = test_input($_POST["achternaam-ouder".$x]);
	}
	  
	if (empty($_POST["email".$x])) {
		$emailErr = "vul een geldig e-mail adres in";
	} else {
		if(!ereg("^[A-Za-z0-9\.|-|_]*[@]{1}[A-Za-z0-9\.|-|_]*[.]{1}[a-z]{2,5}$", ($_POST["email".$x]))) { 
			$emailErr = "vul een geldig e-mail adres in";
		} else { 
	...
	if (empty($_POST["telefoonnummer".$x])) {
		$telefoonErr = "vul een geldig telefoonnummer in";
	} else {
		if(!ereg("^[0]{1}[0-9]{9}$", ($_POST["telefoonnummer".$x]))) { 
			$telefoonErr = "vul een geldig telefoonnummer in";
		} else { 
			$telefoonnummer = test_input($_POST["telefoonnummer".$x]);
		}
	}
...
submitForms = function(){
	<?php for($x = 1; $x <= $_GET['aantal']; $x++) {?>
		document.forms["aanmelding<?php echo "$x" ?>"].submit();
	<?}?>
	document.forms["tos"].submit();
}
</script>
